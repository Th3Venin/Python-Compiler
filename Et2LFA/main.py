...
def duplicate_transition(transition_list, transition):
    for tran in transition_list:
        if tran.current_state == transition.current_state and tran.next_state == transition.next_state \
                and tran.current_value == transition.current_value:
            return True
    return False
...
class Transition:
    ...
    def __init__(self, current_state, current_value, next_state):
...
class NFA:
    ...
    def print_nfa(self):
        ...
        for n in self.transitions:
            print(n.current_state, n.current_value, n.next_state)
...
class Transform:
    ...
    def write_to_file(self, out_file_name):
        ...
        for trans in self.transitions:
            transition = ""
            transition += str(trans.current_state) + ",'" + trans.current_value + "'," + str(trans.next_state)
            f.write("\n")
            f.write(transition)
    ...
    def find_first_set(self):
        ...
        for trans in self.nfa.transitions:
            if trans.current_state in first_set and trans.current_value == "eps":
    ...
    def compute_character_branch(self, character: str, state: int):
        ...
        for trans in self.nfa.transitions:
            if trans.current_state == state and trans.current_value == character:
        ...
        while len(searched_states) > 0:
            ...
            for state in searched_states:
                for trans in self.nfa.transitions:
                    if state == trans.current_state and trans.current_value == "eps":
    ...
    def build_dfa(self):
        ...
        for trans in dfa_transitions:
            print(trans.current_state, trans.current_value, trans.next_state)

        for sett in processed:
            indexes.append(set_to_string(sett))

        for trans in dfa_transitions:
            tr = Transition(indexes.index(trans.current_state), trans.current_value, indexes.index(trans.next_state))
        ...
        for i in range(0, len(processed)):
            ...
            for trans in actual_transitions:
                if trans.current_state == i:
                    characters.append(trans.current_value)
                    nr_of_this_index += 1
        ...
        actual_transitions.sort(key=lambda x: (x.current_state, x.current_value))
